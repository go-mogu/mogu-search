...
import (
	...
	"github.com/go-mogu/mgu-search/pkg/util/text/gregex"
...
func SetTimeZone(zone string) (err error) {
	...
	if err != nil {
		err = errors.Wrapf(err, `time.LoadLocation failed for zone "%s"`, zone)
		return err
	}
	var (
		envKey   = "TZ"
	...
	if err = os.Setenv(envKey, envValue); err != nil {
		err = errors.Wrapf(err, `set environment failed with key "%s", value "%s"`, envKey, envValue)
	}
	return
}
...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, errors.Errorf(`unsupported time converting for string "%s"`, str)
	}
	...
	if month <= 0 || day <= 0 {
		return nil, errors.Errorf(`invalid time string "%s"`, str)
	}
...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if len(fromZone) > 0 {
		if l, err = time.LoadLocation(fromZone[0]); err != nil {
			err = errors.Wrapf(err, `time.LoadLocation failed for name "%s"`, fromZone[0])
			return nil, err
		} else {
...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	if len(match) == 3 {
		...
		if err != nil {
			err = errors.Wrapf(err, `strconv.ParseInt failed for string "%s"`, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = errors.Wrapf(err, `time.ParseDuration failed for string "%s"`, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
