...
import (
	...
	"github.com/go-mogu/mgu-search/pkg/util/gstructs"
...
func Scan(params interface{}, pointer interface{}, mapping ...map[string]string) (err error) {
	...
	if pointerType == nil {
		return errors.New("parameter pointer should not be nil")
	}
...
func doScanList(
	...
	if bindToAttrName == "" {
		return errors.New(`bindToAttrName should not be empty`)
	}
	...
	if reflectKind != reflect.Slice && reflectKind != reflect.Array {
		return errors.Errorf("structSlicePointer should be type of *[]struct/*[]*struct, but got: %v",
			reflectKind,
		)
	}
	length := len(maps)
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return errors.Errorf(`invalid relation fields specified: "%v"`, relationFields)
		}
		...
		default:
			return errors.Errorf(`unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
