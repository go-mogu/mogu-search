...
import (
	...
	"github.com/go-mogu/mgu-search/pkg/util/json"
...
func doStruct(params interface{}, pointer interface{}, mapping map[string]string, priorityTag string) (err error) {
	...
	if v, ok := pointer.(reflect.Value); ok {
		...
		if pointerReflectKind != reflect.Ptr {
			return errors.Errorf("object pointer should be type of '*struct', but got '%v'", pointerReflectKind)
		}
	...
	defer func() {
		if exception := recover(); exception != nil {
			if err = bindVarToReflectValue(structFieldValue, value, mapping); err != nil {
				err = errors.Wrapf(err, `error binding value to attribute "%s"`, attrName)
			}
		}
	}()
	// Directly converting.
